...
function countVowels(str) {
  ...
  vowels.forEach((item) => {
    modStr = modStr.replaceAll(item, '');
  });
  return str.length - modStr.length;
}
...
function isPalindrome(str) {
  const delAlphabet = ['!', ' ', '.', '?', ','];
  let modStr = str;
  delAlphabet.forEach((item) => {
    modStr = modStr.replaceAll(item, '');
  });
  const modStrReverse = modStr.split('').reverse().join('');
  return modStrReverse.toLowerCase() === modStr.toLowerCase();
}
...
function invertCase(str) {
  ...
  arr.forEach((char) => {
    if (char === char.toUpperCase()) {
      invertCaseArr.push(char.toLowerCase());
    } else {
      invertCaseArr.push(char.toUpperCase());
    }
  });
  return invertCaseArr.join('');
}
...
function extractNameFromTemplate(value) {
  return value
    .split(' ')[1]
    .concat(' ')
    .concat(value.split(' ')[2].slice(0, -1));
}
...
function encodeToRot13(str) {
  ...
  let encoded = '';
  for (let i = 0; i < str.length; i += 1) {
    encoded += rot[normal.indexOf(str[i])];
  }
  return encoded;
}
